    ...
    <el-form :model="form2" :rules="rules2" ref="form2" label-width="100px" label-position="left" >
      <div class="tab-container" v-if="tabNum == 2">
        ...
        <el-form-item label="请求示例：" prop="apiResquestExample">
          <el-input type="textarea" size="mini" placeholder="请输入请求示例" class="radius" v-model="form2.apiResquestExample" maxlength="200"></el-input>
        </el-form-item>
        <el-form-item label="接口备注：" prop="apiNote">
          <el-input type="textarea" size="mini" placeholder="请输入接口备注" class="radius" v-model="form2.apiNote" maxlength="500"></el-input>
        </el-form-item>
      </div>
      ...
      <div class="tab-container" v-if="tabNum == 2" >
        ...
        <el-form-item label="" prop="apiResExample" label-width="0">
          <el-input type="textarea" size="mini" class="radius" placeholder="请输入返回示例" v-model="form2.apiResExample"></el-input>
        </el-form-item>
      </div>
    ...
    <el-form  :model="form5" :rules="rules5" ref="form5" label-width="90px" label-position="left" >
      <div class="tab-container" v-if="tabNum == 5">
        <el-form-item label="是否收费:" prop="isFree">
         ...
         <!--  <span class="radio-btn" @click="changeRadio('1')" :class="form5.isFree == '1' ? 'active' : ''">是</span>
          <span class="radio-btn" @click="changeRadio('0')" :class="form5.isFree == '0' ? 'active' : ''">否</span> -->
        </el-form-item>
        <el-form-item label="收费标准:" prop="payStandard" v-if="form5.isFree == '1'">
          <el-input size="mini" v-model="form5.payStandard" placeholder="" class="form-input width-300" maxlength="9">
  ...
  export default {
    ...
    mounted() {
      ...
      if(!this.$route.params.id) {
        this.dataId = localStorage.getItem('idApi')
        this.getDetail()
       
      } else {
        this.dataId = this.$route.params.id
        localStorage.setItem('idApi',this.$route.params.id)
      }
    },
    activated() {
      if (this.$route.params.id) {
    ...
    data() {
      return {
        ...
        rules2: {
          ...
          apiResExample: [
            {required: true, message: '请输入返回示例', trigger: 'blur'}
          ],
        },
        demoRules: {
          title: [
    ...
    methods: {
      ...
      getDetail(){
        ...
        getAllDetail(param).then((resp) => {
          if (resp.code == 200) {
            let data = resp.data[0]
            this.imgList[0].url = data.filePath
            if (data.filePath) {
              this.chooseFileUrl = data.filePath
              this.fileImg = true
              this.delImgVisible = true
            }
            this.form.apiName = data.apiName
            this.form.apiType = data.apiType
            this.apiTypeList = this.form.apiType.split(',')

            this.form.basicType = data.basicType
            ...
            this.form.apiShortDescription = data.apiShortDescription
            this.form.apiDescription = data.apiDescription
            this.form.picAttachmentId = data.picAttachmentId

            this.form2.apiUrl = data.apiUrl
            this.form2.apiResFormat = data.apiResFormat
            this.form2.requestType = data.requestType
            this.form2.apiResquestExample = data.apiRequestExample
            this.form2.apiNote = data.apiNote
            this.form2.requestParametersList = data.requestParametersList.length > 0 ? data.requestParametersList : this.form2.requestParametersList
            this.form2.responseParametersList = data.responseParametersList.length > 0 ? data.responseParametersList : this.form2.responseParametersList
            this.form2.apiResExample = data.apiResExample

            this.form3.sysError = data.systemList
            if (this.form3.sysError.length == 0) {
              this.form3.sysError = [{
                code: '',
                description: '',
                type: '1'
            ...
            this.form3.serviceError = data.serviceList
            if (this.form3.serviceError.length == 0) {
              this.form3.serviceError = [{
                code: '',
                description: '',
                type: '2'
              }]
            }
            this.form3.successCode = data.normalList
            if (this.form3.successCode.length == 0) {
              this.form3.successCode = [{
                code: '',
                description: '',
                type: '3'
              }]
            ...
            this.form4.exampleCallList = data.apiExampleCallVOList

            this.form5.isFree = data.payStandard == '0' ? '0' : '1'
            this.form5.payStandard = data.payStandard
          } 
        })
      },
      changeApiTypeList() {
      ...
      saveStep2() {
        this.$refs.form2.validate((valid) => {
          if (valid) {
            ...
            saveApiStep2(form2Copy).then((resp) => {
              if (resp.code == 200) {
                if (this.isSave) {
                  this.$router.push({
                    name: 'apiCenter',
      ...
      saveStep5() {
        ...
        saveApiStep5(this.form5).then((resp) => {
          if (resp.code == 200) {
            ...
            this.$store.commit('closeTag', 'editApi')
          } else {
            this.$message({
              type: 'error',
              message: resp.message
      ...
      submit() {
        // let formArr = ['form', 'form2'] //需要校验的form
        // let resultArr = []//各个form校验结果
        // let _this = this
        // let checkForm = function(formName) {
        //   let result = new Promise((resove, reject) => {
        ...
        //Promise.all(resultArr).then(() => {
          ...
          param3.statusCodeList = this.form3.sysError.concat(this.form3.serviceError).concat(this.form3.successCode)
          this.form5.payStandard = this.form5.isFree == '1' ? this.form5.payStandard : ''
          let param = Object.assign({}, this.form, this.form2, param3, this.form4, this.form5)
          submitApiAll(param).then(resp => {
            if (resp.code == 200) {
              this.$message({
                type: 'success',
                message: '提交成功'
      ...
      resetParam(data) {
        ...
        this.form2.requestParametersList = data.map((v) => {
          return {
            name: v['名称'],
            requestType: v['请求类型'],
            isRequired: v['必填'] == '是' ? '1' : (v['必填'] == '否' ? '0' : v['必填']),
            type: v['类型'],
            description: v['说明']
          }
        })
