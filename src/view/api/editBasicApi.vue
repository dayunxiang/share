...
<div class="basic-container">
  <p class="page-title">修改基础API</p>
  <div class="main-container">
    <p class="tab">
      <span :class="tabNum == 1 ? 'active' : ''" @click="changeTab(1)">定制API</span>
      <span :class="tabNum == 2 ? 'active' : ''" @click="changeTab(2)">产品详情</span>
    </p>
          ...
          <div>
            <el-table :data="list" border>
              ...
              <el-table-column label="" class-name="text-center">
                <template slot-scope="scope">
                  <el-button size="mini"  @click="addrow" class="plusBtn">
                    <i class="plusBg"></i>
                  </el-button>
                  <el-button size="mini"  v-if="list.length > 1" @click="deleteRow(scope.$index)" class="minusBtn">
                    <i class="minusBg"></i>
                  </el-button>
                </template>
              </el-table-column>
...
<script>
  import {uploadFile, saveBasicApiStep1, saveBasicApiStep2, getBasicApiDetail, submitBasicApi, getApiTypeArray, getBasicTypeArray} from '@/api/api/index'
  ...
  export default {
    ...
    mounted() {
      ...
      if(!this.$route.params.id) {
        this.dataId = localStorage.getItem('idApiBase')
        this.getDetail()
        this.getCategory()
      } else {
        this.dataId = this.$route.params.id
        localStorage.setItem('idApiBase',this.$route.params.id)
      }
    },
    activated() {
      if (this.$route.params.id) {
    ...
    methods: {
      ...
      getDetail() {
        ...
        getBasicApiDetail(param).then(resp => {
          if (resp.code == 200) {
            this.apiDetail = {}
            let data = resp.data

            this.form.objMainId = data.objMainId
            let param = {
              categoryType: this.form.objMainId
            }
            getCategoryById(param).then(response => {
            ...
            this.apiDetail.url = data.url 
            this.apiDetail.requestDescriptionList = data.requestDescriptionList 
            this.apiDetail.dataReturnExampleMap = data.dataReturnExampleMap 
            this.apiDetail.returnExampleMapList = data.returnExampleMapList 

            //this.form2 = {}
            this.form2.apiName = data.apiName
            this.form2.apiType = data.apiType
            this.apiTypeList = this.form2.apiType.split(',')
            this.form2.apiShortDescription = data.apiShortDescription
            this.form2.apiDescription = data.apiDescription
            this.form2.isFree = data.isFree
            this.form2.payStandard = data.payStandard

            this.form2.basicType = data.basicType
            this.basicTypeList = this.form2.basicType.split(',')
            let _this = this
            ...
            this.form2.picAttachmentId = data.picAttachmentId
      ...
      submit() {
        this.$refs.form2.validate(valid => {
          if (valid) {
            this.form2.id = this.dataId
            this.form.sortRuleList = this.list
            let param = Object.assign({}, this.form, this.form2)
            submitBasicApi(param).then((resp) => {
              if (resp.code == 200) {
                this.$message({
                  type: 'success',
                  message: '提交成功'
