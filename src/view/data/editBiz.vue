<template>
	<div>
		<div class="page-title">修改业务数据</div>
        ...
        <div class="tab-container">
          ...
          <div class="preview" id="preview"><img :src="form.examplePicPath" class="upload-image"></div>
        </div>   
      </div>
			<div class="mar-20 info-block">
...
<script>
  ...
  import { uploadFiles, saveBiz, submitBiz, getBizeDetail } from '@/api/data/index'
  import { getType } from '@/api/common/common.js'
    ...
    created() {
      ...
      if(!this.$route.params.id) {
        this.id = localStorage.getItem('idBiz')
        this.getDetail()
      } else {
        this.id = this.$route.params.id
        localStorage.setItem('idBiz',this.$route.params.id)
      }
    },
    activated() {
      if (this.$route.params.id) {
      ...
      getDetail() {
        getBizeDetail(this.id).then(res => {
          let param = res.data
          this.form = {
            id: this.id,
            name: param.name,
            ...
            examplePicId: param.examplePicId,
            examplePicPath: param.examplePicPath,
            type: param.type,
            basicType: param.basicType,
            shortDescription: param.shortDescription,
            isFree: param.isFree,
            payStandard: param.payStandard,
            listAttachmentIds: param.listAttachmentIds
          }
          this.imgList[0].url = param.picPath
          this.delImgVisible = this.imgList[0].url ? true : false
          this.typeList = this.form.type.split(',')
          this.fileList = param.listAttachmentIdsVO ? param.listAttachmentIdsVO : []
      ...
      save() {
       saveBiz(this.form).then(res => {
          if(res.code == 200) {
            ...
            this.$store.commit('closeTag', 'editBiz')
          } else {
            this.$message({
              type: 'error',
              message: res.message,
