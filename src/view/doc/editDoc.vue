...
<script>
  ...
  import { editDoc , getDocDetail } from '@/api/doc/index'
	export default {
		name: 'editDoc',
		data() {
			return {
    ...
    activated() {
      if (this.$route.params.id) {
		...
		methods: {
      getDetail() {
        let param = [getApiTypeArray({type: '2', isChild: '0'}), getDocDetail(this.dataId),  getBasicTypeArray()]
        Promise.all(param).then(resp => {
          this.typeArray = resp[0].data
          this.basicTypeArray = resp[2].data
          this.form = resp[1].data
          this.docFileUrl = resp[1].data.docName
          let type = resp[1].data.type
          //勾选类型
          // this.typeArray.forEach( (v, index) => {
          //   if (type.indexOf(v.codeExt) > -1) {
          //     this.$set(this.typeArray[index], 'checked', true)
          //   }
          // })
          this.typeList = resp[1].data.type.split(',')

          //勾选对象类型
          this.form.basicType =  resp[1].data.basicType
      ...
      getDocDetail() {
        getDocDetail(this.dataId).then(resp => {
          if (resp.code == 200) {
            this.form = resp.data
          }
        })
